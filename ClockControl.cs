...
public class ClockControl : MonoBehaviour
{
    ...
    void UpdateContinuous()
    {
        ...
        hoursTransform.localRotation = Quaternion.Euler(
            0f,
            (float)(time.TotalHours - initialHour) * degreesPerHour,
            0f
        );
        minutesTransform.localRotation = Quaternion.Euler(
            0f,
            (float)(time.TotalMinutes - initialMinute) * degreesPerMinute,
            0f
        );
        secondsTransform.localRotation = Quaternion.Euler(
            0f,
            (float)(time.TotalSeconds - initialSecond) * degreesPerSecond,
            0f
        );
